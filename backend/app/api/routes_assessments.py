...
from ..demo_store import get_demo_profile, insert_demo_assessment, list_demo_assessments
...
async def create_assessment(body: AssessmentIn, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        recommendation = {"resumen": "Recomendación no disponible (LLM)", "factor_principal": "N/A", "recomendaciones": []}
    ...
    if user_id == DEMO_USER_ID:
        saved = insert_demo_assessment(row)
        return saved
...
def list_assessments(user_id: str = Depends(get_current_user_id)):
    if user_id == DEMO_USER_ID:
        return list_demo_assessments(user_id)
